...
yum -y install gcc-c++ gdb git gtest gtest-devel htop unzip valgrind yum-utils yum-plugin-auto-update-debug-info.noarch
...
sudo wget --no-verbose https://google-styleguide.googlecode.com/svn/trunk/cpplint/cpplint.py -O /usr/local/bin/cpplint.py
