...
class CPPCACHE_EXPORT Execution : public SharedBase
{
   ...
   * @param hasResult indicating if results are expected, @deprecated since 3.6, use of this parameter is ignored
   * @param timeout value to wait for the operation to finish before timing out.
   * @param isHA Whether the given function is HA, @deprecated since 3.6, use of this parameter is ignored
   * @param optimizeForWrite Whether the function should be optmized for write operations, @deprecated since 3.6, use of this parameter is ignored
   * @throws Exception if there is an error during function execution
   * @return either a default result collector or one specified by {@link #withCollector(ResultCollector)}
   * @deprecated parameters hasResult, isHA and optimizeForWrite are deprecated since NativeClient 3.6, use of these parameters is ignored.
   */
