...
#define GF_CHECK_NPE(x) if ( x != NULL ) { } else gemfire::SPEHelper::throwNullPointerException( typeid( *this ).name() )
#else
...
public:
  ...
  inline SharedPtr(const NullSharedBase* ptr)
    : m_ptr(NULL)
  {}

  /** Explicit copy constructor, given a pointer.
   * @throws ClassCastException if <code>Target</code> pointer cannot be converted to <code>SharedBase</code> pointer (dynamic_cast to <code>SharedBase*</code> fails).
   */
  inline explicit SharedPtr(const Target* ptr)
    : m_ptr(const_cast<Target *>(ptr))
  {
    if (NULL != m_ptr)
      getSB( m_ptr )->preserveSB();
  }
  ...
  /** Constructor, given another SharedPtr.
   * @throws ClassCastException if <code>Other</code> pointer cannot be converted to <code>Target</code> pointer (dynamic_cast to <code>Target*</code> fails).
   */
  ...
  /** Assigns a pointer of type <code>Other</code> from a <code>SharedPtr</code> object.
   * @throws ClassCastException if <code>Other</code> pointer cannot be converted to <code>Target</code> pointer (dynamic_cast to <code>Target*</code> fails).
   */
  template<class Other>
   ...
   * @throws ClassCastException if <code>Other</code> pointer cannot be converted to <code>Target</code> pointer (dynamic_cast to <code>Target*</code> fails),
   * or if <code>Other</code> pointer cannot be converted to <code>SharedBase</code> pointer (dynamic_cast to <code>SharedBase*</code> fails).
   */
  template<class Other>
...
TargetSP dynCast( const SharedPtr<Other>& other )
{
  ...
  if ( ( other.ptr( ) == NULL ) ) {
    return NULLPTR;
  } else if ( ( otherPtr = dynamic_cast<GF_UNWRAP_SP( TargetSP )*>
        ( other.ptr( ) ) ) != NULL ) {
    return TargetSP( otherPtr );
  } else {
    SPEHelper::throwClassCastException( "dynCast: cast failed",
        typeid( other ).name( ), typeid( TargetSP ).name( ) );
    return NULLPTR;
  }
}
