...
class CPPCACHE_EXPORT EntryEvent : public gemfire::SharedBase
{
  protected:
   ...
   UserDataPtr m_callbackArgument; /**< Callback argument for this event, if any. */
   bool m_remoteOrigin;        /**< True if from a remote (non-local) process */

  public:
  /** Constructor, given all values. */
  EntryEvent( const RegionPtr& region,
  ...
  inline CacheableKeyPtr getKey() const
  {
    return m_key;
  }
   ...
   */
  inline CacheablePtr getOldValue() const
  {
    return m_oldValue;
  }
   ...
   */
  inline CacheablePtr getNewValue() const
  {
    return m_newValue;
  }
