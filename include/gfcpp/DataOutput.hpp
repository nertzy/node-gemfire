...
private:
  ...
  inline static void getEncodedLength( const uint8_t val, int32_t& encodedLen )
  {
    if ( (val == 0) || (val & 0x80) ) {
      // two byte.
      encodedLen += 2;
  ...
  inline static void getEncodedLength( const uint16_t val, int32_t& encodedLen )
  {
    if( val == 0)
    {
      encodedLen += 2;
