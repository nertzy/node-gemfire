...
/* Logs the message if the given level is less than or equal to the current logging level. */
#define GF_LOG(level,expr) if ( level > gemfire::Log::logLevel() ) { } else gemfire::Log::log(level, expr)
...
public:
  ...
  */
  static void init
    ...
    (LogLevel level, const char* logFileName, int32 logFileLimit = 0, int64 logDiskSpaceLimit = 0);
  ...
  */
  ...
  {
    return
      (((s_doingDebug && level == Debug) ||
        GEMFIRE_HIGHEST_LOG_LEVEL >= level) && s_logLevel >= level);
  }
...
public:
  ...
  static void putThrow
    (LogLevel level, const char* msg, const Exception& ex);

  static void putCatch
    (LogLevel level, const char* msg, const Exception& ex);
...
public:
  ...
  {
    if (Log::enabled(m_level))
      Log::enterFn(m_level, m_functionName);
  }

  ~LogFn()
  {
    if (Log::enabled(m_level))
      Log::exitFn(m_level, m_functionName);
  }
...
#define LOGDEBUG if ( gemfire::Log::Debug <= gemfire::Log::logLevel() ) gemfire::LogVarargs::debug

/************************ LOGERROR ***********************************/

#define LOGERROR if ( gemfire::Log::Error <= gemfire::Log::logLevel() ) gemfire::LogVarargs::error

/************************ LOGWARN ***********************************/

#define LOGWARN if ( gemfire::Log::Warning <= gemfire::Log::logLevel() ) gemfire::LogVarargs::warn

/************************ LOGINFO ***********************************/

#define LOGINFO if ( gemfire::Log::Info <= gemfire::Log::logLevel() ) gemfire::LogVarargs::info

/************************ LOGCONFIG ***********************************/

#define LOGCONFIG if ( gemfire::Log::Config <= gemfire::Log::logLevel() ) gemfire::LogVarargs::config

/************************ LOGFINE ***********************************/

#define LOGFINE if ( gemfire::Log::Fine <= gemfire::Log::logLevel() ) gemfire::LogVarargs::fine

/************************ LOGFINER ***********************************/

#define LOGFINER if ( gemfire::Log::Finer <= gemfire::Log::logLevel() ) gemfire::LogVarargs::finer

/************************ LOGFINEST ***********************************/

#define LOGFINEST if ( gemfire::Log::Finest <= gemfire::Log::logLevel() ) gemfire::LogVarargs::finest
