...
namespace gemfire {
  ...
  * PdxInstance are immutable. If you want to change one call {@link #createWriter}.
  */
  ...
  public:    
    ...
    * this PdxInstance. 
    * <p>If a PdxInstance has marked identity fields using {@link PdxWriter#markIdentityField}
    ...
    * @throws IllegalStateException if the field contains an element that is not of CacheableKey derived type. 
    *
    * @see Serializable::registerPdxType
    */    
    ...
    * If a PdxInstance has marked identity fields using {@link PdxWriter#markIdentityField markIdentityField}
    * then only the marked identity fields are its identity fields.
    * Otherwise all its fields are identity fields.
    ...
    * </ol> </p>
    * If an identity field is of type derived from <code>Cacheable</code> then it is deserialized. For deserialization C++ Native Client requires the domain class to be registered.
    * If the deserialized object is an array then all array elements
    * are used to determine equality.
    * If an identity field is of type <code>CacheableObjectArray</code> then it is deserialized and all array elements are used to determine equality.
    * For all other field types the value does not need to be deserialized. Instead the serialized raw bytes are compared and used to determine equality.    
    ...
    */
    ...
    /**
     * @brief serialize this object in gemfire PDX format. This is an internal method.
    ...
    * @throws IllegalStateException if the PdxInstance typeid is not defined yet, to get classname
