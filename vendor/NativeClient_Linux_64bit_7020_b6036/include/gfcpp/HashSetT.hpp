...
namespace gemfire
{
  ...
  template< typename TKEY > class HashSetT
  {
    private:
    ...
    public:
      ...
      class Iterator
      {
        private:
          ...
          inline Iterator( const HashSetOfSharedBase::Iterator& iter )
            : m_iter( iter ) { }
      ...
      inline void swap( HashSetT& other )
      {
       ...
       */
      inline HashSetT( int32_t n )
        : m_set( n, hasher, equal_to )
      {
      }

      /** Copy constructor. */
      inline HashSetT( const HashSetT& other )
        : m_set( other.m_set )
      {
      }
   ...
   */
  class CPPCACHE_EXPORT HashSetOfCacheableKey :
    public _HashSetOfCacheableKey, public SharedBase
  {
  public:
    ...
    inline HashSetOfCacheableKey() :
      _HashSetOfCacheableKey() { }

    /** Creates an empty hash set with at least n buckets. */
    inline HashSetOfCacheableKey(int32_t n) :
      _HashSetOfCacheableKey(n) { }
