...
#define GF_WIDESTRING_HUGE (int8_t)GemfireTypeIds::CacheableStringHuge
  ...
  public:
    ...
    {
      if (isWideString()) {
        throw IllegalStateException("CacheableString::asChar: the string is a "
    ...
    {
      if (isCString()) {
        throw IllegalStateException("CacheableString::asWChar: the string is "
  ...
  protected:
    ...
    char* getASCIIString(const wchar_t* value, int32_t& len,
        int32_t& encodedLen);
    /** Default constructor. */
  ...
  {
    return (value != NULL ? CacheableKeyPtr(CacheableString::create(
        value).ptr()) : NULLPTR);
  }
  ...
  {
    return (value != NULL ? CacheablePtr(CacheableString::create(
        value).ptr()) : NULLPTR);
  }
