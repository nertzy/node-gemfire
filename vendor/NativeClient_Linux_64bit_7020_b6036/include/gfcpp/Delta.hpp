...
namespace gemfire {
   ...
   * To use delta propagation, an application class must derive from <code>class Delta</code> as well as <code>class Cacheable</code> publicly.
   * The methods <code>hasDelta( ), toDelta( )</code> and <code>fromDelta( )</code> must be implemented by the class, as these methods are used by GemFire
  ...
  public:
    ...
    /**
     * <code>hasDelta( )</code> is invoked by GemFire during <code>Region::put( CacheableKeyPtr, CacheablePtr )</code> to determine if the object contains a delta.
     * If <code>hasDelta( )</code> returns true, the delta in the object is serialized by invoking <code>Delta::toDelta( DataOutput& )</code>.
     * If <code>hasDelta( )</code> returns false, the object is serialized by invoking <code>Cacheable::toData( DataOutput& )</code>.
     */
     ...
     *
     * @throws IOException
     * @throws InvalidDeltaException if the delta in the <code>DataInput</code> instance cannot be applied
     ...
     * The default implementation of this method creates an object clone by first serializing the object into
     * a buffer, then deserializing from the buffer thus creating a clone of the original.
     */
